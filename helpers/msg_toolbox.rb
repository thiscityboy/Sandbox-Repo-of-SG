...
#
=end
  def self.subscribe(campaignId, attribute_values, custom_attributes)
    @url = 'http://www.vibescm.com/api/subscription_campaigns/'+campaignId.to_s+'/multi_subscriptions.xml'
    @payload = "<?xml version='1.0' encoding='UTF-8'?><subscriptions><opt_in>invite</opt_in>"

    if !custom_attributes == nil
      "<create_attribute>true</create_attribute>"
    end
    ...
    if !custom_attributes == nil
      ...
      custom_attributes.each_pair do |key, value|
        @payload << "<attribute_path>" + key + "/" + value +"</attribute_path>"
      end
      @payload << "</attribute_paths>"
    end
